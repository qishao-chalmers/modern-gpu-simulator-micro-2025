#!/usr/bin/env bash
# SM90_H100_best_optC: remodel memory/issue + TC only (DRAM/L2 unchanged)
set -euo pipefail
...
"$ROOT/gpu-simulator/bin/release/accel-sim.out" \
  -config "$ROOT/gpu-simulator/gpgpu-sim/configs/tested-cfgs/SM90_H100_best_optC/gpgpusim.config" \
  ...
  -trace "$TRACE" \
  | tee "$ROOT/log/prefill_best_multiopt/SM90_H100_best_optC.log"
