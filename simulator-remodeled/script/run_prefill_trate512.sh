#!/usr/bin/env bash
# Experiment: tensor_rate_per_cycle 2048→512
# Compare gpu_sim_cycle against log/prefill/sim.log (baseline SM90_H100)
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

mkdir -p "$ROOT/log/prefill_trate512"
...
"/home/qshao/Project/Fun/modern-gpu-simulator-micro-2025/simulator-remodeled/gpu-simulator/bin/release/accel-sim.out" \
  -config "$ROOT/gpu-simulator/gpgpu-sim/configs/tested-cfgs/SM90_H100_trate512/gpgpusim.config" \
  ...
  | tee "$ROOT/log/prefill_trate512/sim.log"
