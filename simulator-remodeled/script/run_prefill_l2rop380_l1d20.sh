#!/usr/bin/env bash
# Experiment: perfect inst/const cache ON; l2_rop 380 + memory_l1d_minimum_latency 20 (DRAM fixed)
# Compare gpu_sim_cycle against log/prefill/sim.log (baseline SM90_H100)
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

mkdir -p "$ROOT/log/prefill_l2rop380_l1d20"

OMP_NUM_THREADS=16 OMP_PROC_BIND=spread \
"$ROOT/gpu-simulator/bin/release/accel-sim.out" \
  -config "$ROOT/gpu-simulator/gpgpu-sim/configs/tested-cfgs/SM90_H100_l2rop380_l1d20/gpgpusim.config" \
  ...
  | tee "$ROOT/log/prefill_l2rop380_l1d20/sim.log"
