#!/usr/bin/env bash
# Experiment: L1D MSHR entries 512 → 1024, miss queue 16 → 64
# Compare against log/prefill/sim.log (baseline SM90_H100)
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

mkdir -p "$ROOT/log/prefill_mshr1024_mq64"

OMP_NUM_THREADS=16 OMP_PROC_BIND=spread \
"$ROOT/gpu-simulator/bin/release/accel-sim.out" \
  -config "$ROOT/gpu-simulator/gpgpu-sim/configs/tested-cfgs/SM90_H100_mshr1024_mq64/gpgpusim.config" \
  ...
  | tee "$ROOT/log/prefill_mshr1024_mq64/sim.log"
